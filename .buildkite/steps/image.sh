...
HOME=/creds docker buildx build --tag="ghcr.io/superorbital/ruby-docker-example:${BUILDKITE_COMMIT}" --platform=linux/arm64 --push --metadata-file="$(pwd)/metadata.json" --build-arg REPO="${BUILDKITE_REPO}" .
